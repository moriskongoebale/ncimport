...
if (isset($_POST['priority_submit']) AND isset($_POST['id'])) {
$_POST['id'] = isset($_POST['id']) ? trim(preg_replace("/[^0-9]/","", $_POST['id'])) : abDie('id');
$_POST['priority'] = isset($_POST['priority']) ? (int)trim(preg_replace("/[^0-9]/","", $_POST['priority'])) : abDie('priority');
$_POST['table'] = isset($_POST['table']) ? trim(preg_replace("/[^a-z0-9_]/","", $_POST['table'])) : abDie('table');

$del = $antibot_db->exec("UPDATE ".$_POST['table']." SET priority = '".$_POST['priority']."' WHERE rowid=".$_POST['id'].";");
